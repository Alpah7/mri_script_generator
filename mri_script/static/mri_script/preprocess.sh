for sub in {1..$numsub}; do
    cd $sub

  for scan in {1..$numscan}; do
    cd $scan
    ...
    echo "--Import Functional Scan--"
        ...
        for i in {1..$numrun}; do
          if [ ! -f run${i}+orig.HEAD ]; then
            ...
            to3d -time:zt ${slice} $run1TRs ${lengthtr} alt+z -prefix run${i} raw/EPI_Run_*/*.dcm
          fi
        done
    ...
    echo "--Motion Correction Within Functional Run--"
        ...
        halfnumtr=$numtr/2
        3dvolreg -base run1_shift+orig'[$halfnumtr]' -prefix run1_volreg -1Dfile motion_1 run1_shift+orig
